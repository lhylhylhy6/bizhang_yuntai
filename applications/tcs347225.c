...
#define t_scl GET_PIN(B,10)
#define t_sda GET_PIN(B,11)

#define TCS_SCL_H     rt_pin_write(t_scl, PIN_HIGH)
#define TCS_SCL_L     rt_pin_write(t_scl,PIN_LOW)
#define TCS_SDA_H     rt_pin_write(t_sda,PIN_HIGH)
#define TCS_SDA_L     rt_pin_write(t_sda,PIN_LOW)
/******************************************************************************/
...
{
    rt_pin_mode(t_scl, PIN_MODE_OUTPUT);
    rt_pin_mode(t_sda, PIN_MODE_OUTPUT);
    rt_pin_write(t_scl, PIN_HIGH);
    rt_pin_write(t_sda, PIN_HIGH);
}
/*********************************************/
...
//返回值：1，接收应答失败
//        0，接收应答成功
rt_uint8_t TCS34725_I2C_Wait_ACK()
{
    rt_uint32_t t=0;

    TCS_SDA_IN();//SDA设置为输入
    TCS_SDA_H;
    rt_hw_us_delay(1);//delay_us(1);
    TCS_SCL_H;
    rt_hw_us_delay(1);//delay_us(1);
    while(TCS_SDA_READ)
    {
...
/*********************************************/
//I2C发送一个字节
void TCS34725_I2C_Send_Byte(rt_uint8_t byte)
{
    u8 i;
...
rt_uint8_t TCS34725_I2C_Read_Byte(rt_uint8_t ack)
{
    u8 i,receive = 0;
...
*******************************************************************************/
void TCS34725_I2C_Write(rt_uint8_t slaveAddress, rt_uint8_t* dataBuffer,rt_uint8_t bytesNumber, rt_uint8_t stopBit)
{
    rt_uint8_t i = 0;
 ...
 *                           1 - A stop condition will be sent.
*******************************************************************************/
void TCS34725_I2C_Read(rt_uint8_t slaveAddress, rt_uint8_t* dataBuffer, rt_uint8_t bytesNumber, rt_uint8_t stopBit)
{
    rt_uint8_t i = 0;
 ...
 * @return None.
*******************************************************************************/
void TCS34725_Write(rt_uint8_t subAddr, rt_uint8_t* dataBuffer, rt_uint8_t bytesNumber)
{
    u8 sendBuffer[10] = {0, };
    u8 byte = 0;
 ...
 * @return None.
*******************************************************************************/
void TCS34725_Read(rt_uint8_t subAddr, rt_uint8_t* dataBuffer, rt_uint8_t bytesNumber)
{
